...
pub struct DialogEffectsReferenceConfig {
    ...
    pub apply_temporary_model_id: Option<u32>,
    #[serde(default)]
    pub remove_temporary_model: bool,
            ...
            if let Some(dialog) = &config.new_dialog {
                packets.push(GamePacket::serialize(&TunneledPacket {
                    ...
                    inner: EnterDialog {
                        ...
                        speaker_guid: dialog.speaker_guid.unwrap_or(0),
                        enable_escape: true,
                        unknown4: 0.0,
                        dialog_choices: dialog.choices.iter().map(|choice| DialogChoice {
...
impl DialogInstance {
    pub fn from_template(
        ...
        let speaker_guid = template.npc_key.as_ref().map(|key| {
