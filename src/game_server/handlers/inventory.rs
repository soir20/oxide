...
use super::{
    character::{Character, CharacterType}, lock_enforcer::CharacterLockRequest, unique_guid::player_guid,
};
...
fn equip_item_in_slot(sender: u32, equip_guid: EquipGuid, mut characters_write: BTreeMap<u64, RwLockWriteGuard<Character>>, game_server: &GameServer) -> Result<Vec<Broadcast>, ProcessPacketError> {
    if let Some(character_write_handle) = characters_write.get_mut(&player_guid(sender)) {

        let mut wield_type = character_write_handle.wield_type();
        let broadcasts = if let CharacterType::Player(ref mut player_data) = character_write_handle.character_type {
