...
pub enum AttackCruiserPlayerReadiness {
    #[default]
    Initializing,
    WaitingForReadySignal,
    Ready,
}

impl AttackCruiserPlayerReadiness {
    pub fn next(&self) -> Self {
        match self {
            AttackCruiserPlayerReadiness::Initializing => {
                AttackCruiserPlayerReadiness::WaitingForReadySignal
            }
            AttackCruiserPlayerReadiness::WaitingForReadySignal => {
                AttackCruiserPlayerReadiness::Ready
            }
            AttackCruiserPlayerReadiness::Ready => AttackCruiserPlayerReadiness::Ready,
        }
    }
}
...
impl AttackCruiserGame {
            ...
            GamePacket::serialize(&TunneledPacket {
                ...
                inner: AttackCruiserClientConfig {
                    ...
                    game_config: AttackCruiserStartupConfig::new(
                        ...
                        AttackCruiserStartupConfigDefinition::Game(Box::new(
    ...
    pub fn update_client_players(
        ...
        let broadcasts = match self.player_states[player_index as usize].readiness {
            AttackCruiserPlayerReadiness::Initializing => {
                self.add_player_to_client(sender, player_index, update_type)
            }
            AttackCruiserPlayerReadiness::WaitingForReadySignal => {
                ...
                if !self.is_singleplayer() {
                    ...
                    broadcasts.append(&mut self.add_player_to_client(
                        sender,
                        other_player_index,
                        update_type,
                    )?);
            ...
            AttackCruiserPlayerReadiness::Ready => {
                self.update_client_players_once_ready(sender, update_type)
            }
        }?;

        let player_state = &mut self.player_states[player_index as usize];
        player_state.readiness = player_state.readiness.next();
    ...
    fn add_player_to_client(
        ...
        let AttackCruiserGameState::WaitingForPlayersReady = &self.state else {
            return Err(ProcessPacketError::new(
                ...
                format!("Tried to add player index {player_index} to client {sender}, but the game isn't waiting for readiness ({self:?})")
            ));
        };
